...
class GetMostPlayedGameSummariesUseCaseTest {
    ...
    void getMostPlayedGameSummaries_success() {
        ...
        given(gameService.getMostPlayedGameSummaries(userId, categoryId, limit)).willReturn(summaries);

        // when
        List<GameSummary> result = getMostPlayedGameSummariesUseCase.getMostPlayedGameSummaries(userId, categoryId, limit);

        // then
        assertThat(result).hasSize(1);
        assertThat(result.getFirst().name()).isEqualTo("Catan");
        verify(gameService).getMostPlayedGameSummaries(userId, categoryId, limit);
    }
}
